...
import { ForbiddenError } from "../services/projectService.js";

export async function errorHandler(err: Error, c: Context) {
  if (err instanceof HTTPException) {
    return c.json({ error: "http", message: err.message }, err.status);
  }
  if (err instanceof OkfStrictError) {
    return c.json({ error: "okf_strict", message: err.message }, 422);
  }
  if (
    err instanceof UnknownSettingError ||
    err instanceof ImmutableSettingError
  ) {
    return c.json({ error: "invalid_setting", message: err.message }, 400);
  }
